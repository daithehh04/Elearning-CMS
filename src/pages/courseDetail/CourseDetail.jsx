...
import { requestLoadCourseById } from '../../stores/middlewares/courseMiddleware';
import styles from './CourseDetail.module.scss';
import { LessonCourse } from './LessonDetail';
import { setDataTopic } from '../../stores/slices/topicSlice';
import {
...
const CourseDetail = () => {
  ...
  const [topicParentList, setTopicParentList] = useState([]);
  const [activeTopic, setActiveTopic] = useState([]);
  const [indexActive, setIndexActive] = useState();
  const [indexActiveDataChild, setIndexActiveDataChild] = useState();
  ...
  const loadTopicsByCourse = useCallback(async (idCourse, type, parentId) => {
    try {
  ...
  const loadCourseById = async (params) => {
    try {
  ...
  const handleDrapEnd = async (result) => {
  ...
  const handleDrapEndTopicChild = async (result) => {
    const idTopicChild = result.draggableId;
    const topicParent = topicParentList?.find((topic) =>
      topic?.topicChildData?.find((o) => o?.id === idTopicChild)
    );
    let topicChild = _.sortBy(
      topicParent?.topicChildData,
      [
        (o) => {
          return o.index;
        },
    ...
    dispatch(
      setDataTopic({
        ...
        index:
          key === '0'
            ? (topics.length || 0) + 1
            : (parent?.topicChildData.length || 0) + 1,
      })
    );
    setIsOpenEdit(true);
  };

  const handleClickTopicParent = (topic, index) => {
  ...
  const handleClickTopicChild = async (topicChild, indexActive) => {
    setIndexActive(undefined);
    setIndexActiveDataChild(indexActive);
    setIsOpenEdit(true);
    try {
      const requestResult = await dispatch(
        requestLoadTopicById({ id: topicChild?.id || '' })
      );
    ...
    <div>
      ...
      <Row gutter={24}>
          ...
          >
            ...
            <Col span={4}>
              ...
              >
                ...
                >
                  <button className={clsx(styles.dropdownBtn)}></button>
                </a>
              </Dropdown>
            </Col>
          </Row>
                  ...
                  {(provided, snapshot) => (
                    <div ref={provided.innerRef} {...provided.droppableProps}>
                      <Collapse
                        ...
                        collapsible="icon"
                      >
                        {topicParentList?.length > 0 &&
                          topicParentList?.map((topic, i) => {
                            return (
                              <Collapse.Panel
                                style={{
                                  alignItems: 'center',
                                  backgroundColor:
                                    i === indexActive ? '#caf0ff' : '',
                                }}
                                header={
                                  <Draggable
                                    key={topic?.id}
                                        ...
                                        <div id={topic?.id} key={i}>
                                          ...
                                          >
                                            <Col
                                              span={20}
                                              onClick={() =>
                                                handleClickTopicParent(topic, i)
                                              }
                                            >
                                              {topic.name}
                                            </Col>
                              ...
                              >
                                <DragDropContext
                                  onDragEnd={handleDrapEndTopicChild}
                                >
                                              ...
                                              >
                                                <Row
                                                  style={{
                                                    ...
                                                    cursor: 'pointer',
                                                    backgroundColor:
                                                      indexActiveDataChild ===
                                                      `${i}:${index}`
                                                        ? '#caf0ff'
                                                        : topicChild.status ===
                                                          TTCSconfig.STATUS_PRIVATE
                                                        ? '#fbffca'
                                                        : '',
                                                  }}
                                                  onClick={() =>
                                                    handleClickTopicChild(
                                                      topicChild,
                                                      `${i}:${index}`
                                                    )
                                                  }
        ...
        <Col span={18} style={{ backgroundColor: '#f7f7f7' }}>
          {isOpenEdit && (
            <LessonCourse
              onloadTopic={loadTopicsByCourse}
              type={type}
              setIndexActiveDataChild={setIndexActiveDataChild}
